...
// Addition of bad, which significate that we used the "joker:
fn is_valid_rec(line: List(Int), order: Option(order.Order), bad: Bool) {
  case line {
    ...
    [n1, n2, ..tail] -> {
      // case 1, good
      {
        ...
        && is_valid_rec([n2, ..tail], o, bad)
      }
      // case 2: joker, pass bad to false
      || { bad == False && is_valid_rec([n2, ..tail], order, True) }
    }
  }
}
...
pub fn main() {
  ...
  input
  |> parse_input
  |> list.count(is_valid_rec(_, None, False))
  |> io.debug
}
